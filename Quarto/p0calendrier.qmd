...
#Tri du tableau par date puis NoPartie
  calendrier <- calendrier %>%
  arrange(Date, NoPartie)
...
reactable(
  ...
  columns = list(
    ...
    # 1. Obtenez le numéro de partie réel (NoPartie) du tableau 'calendrier'
    no_partie_reel <- calendrier$NoPartie[index] 
    
    # 2. Utilisez ce numéro pour construire le libellé (Texte)
    Texte <- sprintf("Lien joueur #%s", no_partie_reel) 
...
"LienReunion" = colDef(
  name = "Lien Réunion",
  cell = function(value, index) {
    # 1. Obtenez le NoPartie de la ligne ACTUELLE (triée) dans le tableau 'calendrier'
    no_partie_reel <- calendrier$NoPartie[index] 

    if (!is.na(value) && grepl("^https?://", value)) {
      # 2. Utilisez ce NoPartie pour construire le libellé (Texte)
      Texte <- sprintf("Lien Teams #%s", no_partie_reel) 
