...
calendrierDetailJoueurs_series <- merge(RépondantsParJoueurs_series,joueurs, by = c("NoÉquipe", "NoJoueur") )   
calendrierDetailJoueurs_series <- merge(calendrierDetailJoueurs_series,équipes, by = c("NoÉquipe")) 
calendrierDetailJoueurs_series <- calendrierDetailJoueurs_series[,c("NoPartie", "PrénomJoueur","NomJoueur","NomÉquipe","Points")] 
calendrierDetailJoueurs_series <- calendrierDetailJoueurs_series %>% 
...
calendrier_series <- left_join(calendrier_series, équipes, by = c("NoÉquipeQuestionnaire" = "NoÉquipe")) %>% 
...
AnimateurConsole <- colDef(
  ...
  cell = function(value, index, column_name) {
    # Création de l'URL.
    url <- calendrier_series[index, "LienConsole"]
...
LienQuestionnaire <- colDef(
  ...
  cell = function(value, index, column_name) {
    if (!is.na(value)) {
      url2 <- paste0(lien_questionnaire,"match",calendrier_series[index, "NoPartie"],".htm")
...
#Ordre et sélection des colonnes
  ...
  calendrier_series <- calendrier_series %>%
...
reactable(
  #Il faut enlver les colonnes utilisé pour les URL
  calendrier_series[, !(names(calendrier_series) %in% NomColonneAExclure)], #exclusion de la colonne LienConsole
  bordered = FALSE,
  highlight = TRUE,
  striped = TRUE,
  pagination = FALSE,
 ...
 #theme : Permet plus de personnalisation de reactable. Requis pour mettre la recherche à gauche.
  ...
  details = function(index) {
    detailcalendrier <- calendrierDetailJoueurs_series[calendrierDetailJoueurs_series$NoPartie == calendrier_series$NoPartie[index], ]
